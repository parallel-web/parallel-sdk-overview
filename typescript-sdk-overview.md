Below is a **single `index.d.ts`** file that summarizes the **public API footprint** exposed by this SDK (as seen in `src/index.ts`, `src/client.ts`, and all public `src/resources/**` types). I copied doc-comments **1:1** where they exist in the provided sources. (Internal-only modules are not included except where their types are explicitly re-exported publicly.)

> Notes:
> - This is a “summary” d.ts: it focuses on public surface area (classes, methods, exported types).
> - Paths/imports are removed; everything is written in one file.
> - Some runtime-only implementation details (like actual method bodies) are intentionally omitted.

```ts
// index.d.ts
// Single-file public API footprint summary for the Parallel TypeScript SDK.

declare const VERSION: '0.2.4'; // x-release-please-version
...
export class APIPromise<T> extends Promise<T> {
  constructor(
    ...
    responsePromise: Promise<{
      response: Response;
      options: RequestOptions & { method: HTTPMethod; path: string };
      controller: AbortController;
      requestLogID: string;
...
export class TaskGroupResource extends APIResource {
  /**
...
export type FindallRetrieveResponse = FindallRun | {
  ...
  results: Array<{
    ...
    name: string;
    description?: string | null;
    enrichment_results?: Array<any>;
    filter_results?: Array<any>;
    score?: number | null;
    url?: string | null;
  }>;
  ...
  created_at?: string | null;
  enrichment_recommendations?: Array<any>;
  modified_at?: string | null;
  pages_considered?: number | null;
  pages_read?: number | null;
};

export interface FindallCreateParams extends FindallRunInput {
  /**
   * Header param: Optional header to specify the beta version(s) to enable.
   */
  betas?: Array<ParallelBeta>;
}
export interface FindallRetrieveParams { betas?: Array<ParallelBeta>; }
export interface FindallCancelParams { betas?: Array<ParallelBeta>; }
export interface FindallEnrichParams extends FindallEnrichInput { betas?: Array<ParallelBeta>; }
export interface FindallEventsParams { last_event_id?: string | null; timeout?: number | null; betas?: Array<ParallelBeta>; }
export interface FindallExtendParams extends FindallExtendInput { betas?: Array<ParallelBeta>; }
export interface FindallIngestParams extends IngestInput { betas?: Array<ParallelBeta>; }
export interface FindallResultParams { betas?: Array<ParallelBeta>; }
export interface FindallSchemaParams { betas?: Array<ParallelBeta>; }
...
export class Parallel {
  ...
  request<Rsp>(options: (RequestOptions & { method: HTTPMethod; path: string }) | Promise<(RequestOptions & { method: HTTPMethod; path: string })>): APIPromise<Rsp>;
...
1) generate a **second “LLM-optimized” view** (flattened list of endpoints with method/path + request/response types), or  
2) make this file **match the exact TS namespacing** patterns used by Stainless generators (e.g., `TaskRun.TaskRun`, `Beta.TaskGroup`, etc.).