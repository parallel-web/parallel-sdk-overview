...
// NOTE: This is a *summary* of the public API footprint of the provided source tree.
// It aims to be LLM-readable, not a byte-for-byte re-export map.
// Doc-comments that were present in the sources are copied over 1:1 where included below.
...
declare class Parallel {
  ...
  /**
   * Build a full URL from a path, query, and optional defaultBaseURL.
   */
  buildURL(path: string, query: Record<string, unknown> | null | undefined, defaultBaseURL?: string): string;

  static Parallel: typeof Parallel;
  static DEFAULT_TIMEOUT: number; // 1 minute
...
declare namespace Parallel {
  ...
  export type NullableHeaders = {
    ...
    nulls: Set<string>;
    /** Brand check; not constructible from userland. */
  ...
  export type RequestOptions = {
    ...
    __streamClass?: typeof Stream<any>;
  };
  ...
  export type Uploadable = File | Response | (AsyncIterable<Uint8Array> & { path: string | { toString(): string } }) | (Blob & { name?: string | undefined });
  ...
  export class BetaFindAllResource extends APIResource {
    /**
  ...
  // Deprecated Findall* aliases (beta/findall.ts)
...
export { Parallel };
export type {
  Parallel as ParallelClient,
  Parallel as ParallelNamespace,
  // Convenience re-exports commonly exposed by index.ts:
  ParallelError,
  APIError,
  APIConnectionError,
