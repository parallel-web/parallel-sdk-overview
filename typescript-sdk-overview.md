Below is a single **`parallel-sdk.d.ts`** file that summarizes the **public** footprint of the SDK as shown in your `/src` tree. It’s essentially a flattened “API surface” view meant for an LLM to understand how to use the client, what resources exist, and what types are involved.

Notes / constraints (kept brief):
- I only included **publicly importable** modules (i.e., what `/src/index.ts` exports, plus the resource/type exports reachable from there). I did **not** include `/internal/**` because it is explicitly non-public.
- I copied doc-comments **1:1 where present** in the public files you provided (notably in `client.ts`, `resources/*.ts`, `core/*`).
- This is a *summary* d.ts, not guaranteed to be byte-for-byte identical to TypeScript’s emitted `.d.ts` from building the package, but it captures the full usable API footprint.
...
export interface TaskRunCreateParamsBeta {
  /**
...
export interface TaskRunResultParamsBeta {
  /**
   * Query param
   */
  timeout?: number;
...
export declare class BetaTaskRunResource extends APIResource {
   ...
   * Beta features can be enabled by setting the 'parallel-beta' header.
   */
  create(params: TaskRunCreateParamsBeta, options?: Parallel.RequestOptions): APIPromise<TaskRun>;
  ...
  result(
    runID: string,
    params?: TaskRunResultParamsBeta | null | undefined,
    options?: Parallel.RequestOptions,
  ): APIPromise<BetaTaskRunResult>;
}

/* Beta TaskGroup types & resource */
...
export declare class TaskGroupResource extends APIResource {
  /**
   ...
   */
  addRuns(taskGroupID: string, params: TaskGroupAddRunsParams, options?: Parallel.RequestOptions): APIPromise<TaskGroupRunResponse>;
...
/* Beta FindAll types & resource (surface) */
...
/* Namespace re-exports (as in src/client.ts) */
export declare namespace Parallel {
  export type RequestOptions = import('./parallel-sdk').RequestOptions;
...
If you want, I can also generate a **second** “LLM-optimized” `.d.ts` variant that:
- collapses duplicate types (`TaskRun` name collision between stable resource class and type),
- groups methods by endpoint path,
- adds a small “usage” doc section (without altering existing doc-comments), and
- removes all implementation-irrelevant static error aliases.